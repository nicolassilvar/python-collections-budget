...
class Expenses():
    ...
    def categorize_set_comprehension(self):
            neccesary_expenses = {x for x in self.list if x.category == 'Phone' or x.category == 'Auto and Gas' or x.category == 'Classes' or x.category == 'Utilities' or x.category == 'Mortgage'}
            food_expenses = {x for x in self.list if x.category == 'Groceries' or x.category == 'Eating out'}
            unnecessary_expenses = set(self.list) - neccesary_expenses - food_expenses
            return necessary_expenses, food_expenses, unnecessary_expenses
